...
# python项目作为Client通过接口或其他方式被调用，获取资源数据。对IdToken进行校验；
# 必须配置issuer信息，Resource端可以不配置client_id；Client端一定要配置client_id
# Client端需要校验client_id；
...
from flask_oidc_validator.tools import base64_urldecode
from flask_oidc_validator.tools import get_ssl_context
...
class IdTokenValidatorForRs(object):
    ...
    def __init__(self, config, app=None):
        ...
        if self.config['client_id'] is None or len(self.config['client_id']) == 0:
            raise ValueError('The expected client_id must not be null')
    ...
    def jwtValidate(self, token):
        ...
        # 校验 client_id
        if payload["aud"]:
            if (isinstance(payload["aud"], str) and payload["aud"] != self.clientId) or self.clientId not in payload['aud']:
                return "Invalid audience %s, expected %s" % (payload['aud'], self.clientId)
        # 校验过期时间
